...
class JWTAuthenticationTests(TestCase):
    ...
    def test_logout_and_token_blacklisting(self):
        ...
        token_is_blacklisted = BlacklistedToken.objects.filter(token_jti=token_jti).exists()
    ...
    def test_expired_token_cleanup(self):
        ...
        self.assertEqual(BlacklistedToken.objects.filter(token_jti__in=['expired_test_token', 'valid_test_token']).count(), 2)
        
        # Run cleanup
        BlacklistedToken.clean_expired_tokens()
        
        # Verify only non-expired token remains
        self.assertEqual(BlacklistedToken.objects.filter(token_jti='valid_test_token').count(), 1)
        self.assertTrue(BlacklistedToken.objects.filter(token_jti='valid_test_token').exists())
        self.assertFalse(BlacklistedToken.objects.filter(token_jti='expired_test_token').exists())
    ...
    def tearDown(self):
        ...
        BlacklistedToken.objects.filter(token_jti__in=['expired_test_token', 'valid_test_token']).delete()
...
class BlacklistedTokenModelTests(TestCase):
    ...
    def setUp(self):
        ...
        BlacklistedToken.objects.filter(token_jti__in=[
            'test_token_jti', 'non_existent_token',
            'expired_token', 'valid_token'
        ]).delete()
    ...
    def test_clean_expired_tokens(self):
        ...
        self.assertEqual(BlacklistedToken.objects.filter(token_jti__in=['expired_token', 'valid_token']).count(), 1)
        self.assertTrue(BlacklistedToken.objects.filter(token_jti='valid_token').exists())
        self.assertFalse(BlacklistedToken.objects.filter(token_jti='expired_token').exists())
