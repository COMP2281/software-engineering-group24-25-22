...
class MongoDBAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        header_html = "".join([f"<th scope='col' class='column-{field}'>{self.get_header_for_field(field)}</th>" 
            ...
            for field_index, field_name in enumerate(self.list_display):
                if hasattr(self, field_name) and callable(getattr(self, field_name)):
                    ...
                    if hasattr(method, 'is_reference') and method.is_reference:
                        ...
                        ref_obj = value
                        if ref_obj:
                            ref_value = method.reference_display_func(ref_obj) if hasattr(method, 'reference_display_func') else str(ref_obj)
                            if hasattr(ref_obj, 'id'):
                                ref_app = getattr(method, 'reference_app', app_label)
                                ref_model = getattr(method, 'reference_model', ref_obj.__class__.__name__.lower() + 'proxy')
                                row_cells.append(f"<td><a href='/admin/{ref_app}/{ref_model}/{ref_obj.id}/'>{ref_value}</a></td>")
                            else:
    ...
    @classmethod
    def reference_field(cls, field_func=None, *, app=None, model=None, display_func=None):
        """
    ...
    def detail_view(self, request, object_id):
        ...
        for field_name in self.list_display:
            if hasattr(self, field_name) and callable(getattr(self, field_name)):
                ...
                if field_name != 'get_display_data':  # Skip the raw data display
                    label = getattr(method, 'short_description', field_name.replace('get_', '').replace('_', ' ').title())
                    value = method(obj)
                    ...
                    if hasattr(method, 'is_reference') and method.is_reference:
                        ...
                        if ref_obj:
                            ...
                            if hasattr(ref_obj, 'id'):
                                ref_app = getattr(method, 'reference_app', app_label)
                                ref_model = getattr(method, 'reference_model', ref_obj.__class__.__name__.lower() + 'proxy')
                                all_fields[label] = f"<a href='/admin/{ref_app}/{ref_model}/{ref_obj.id}/'>{ref_value}</a>"
                            else:
                        ...
                        icon = "✓" if value else "✗"
                        all_fields[label] = f"<span style='color:{'green' if value else 'red'}'>{icon}</span>"
                    else:
        ...
        for field_name in dir(obj):
            ...
            if (field_name.startswith('_') or 
                field_name in ('objects', 'DoesNotExist', 'MultipleObjectsReturned') or
                callable(getattr(obj, field_name))):
                continue
            ...
            if value is None:
                ...
                import json
                try:
                    formatted_value = f"<pre>{json.dumps(value, default=str, indent=2)}</pre>"
                except:
                    formatted_value = f"<pre>{str(value)}</pre>"
            elif isinstance(value, bool):
                icon = "✓" if value else "✗"
                formatted_value = f"<span style='color:{'green' if value else 'red'}'>{icon}</span>"
            else:
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path('<path:object_id>/', self.admin_site.admin_view(self.detail_view), name=f'{self.model._meta.app_label}_{self.model._meta.model_name}_change'),
        ]
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        if hasattr(response, 'content') and isinstance(response.content, bytes):
            ...
            for row_html in rows:
                ...
                email_link = f'<a href="/admin/accounts/userproxy/{user_id}/">{email}</a>'
                
                # Replace the plain email with the link in the row HTML
                new_row_html = row_html.replace(f'<td>{email}</td>', f'<td>{email_link}</td>', 1)
...
class EmployeeProfileAdmin(MongoDBAdmin):
    ...
    list_display = ('get_id', 'get_employee_id', 'get_name', 'get_department', 'get_position', 'get_user', 'get_manager')
...
class ExpenseSettingsAdmin(MongoDBAdmin):
    ...
    list_display = ('get_id', 'get_user', 'get_currency', 'get_expense_limit', 'get_approver')
    # Remove list_filter for now as it causes issues with proxy models
    list_filter = ()
