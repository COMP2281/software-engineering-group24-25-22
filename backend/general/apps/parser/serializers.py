...
class ReceiptUploadSerializer(serializers.Serializer):
    ...
    file = serializers.FileField(
        validators=[FileExtensionValidator(allowed_extensions=['jpg', 'jpeg', 'png', 'pdf', 'tiff'])]
    )
...
class CostItemSerializer(serializers.Serializer):
    ...
    quantity = serializers.DecimalField(max_digits=8, decimal_places=2, required=False, default=1.0)
    ...
    def validate(self, attrs):
        ...
        if 'total_price' not in attrs and ('unit_price' not in attrs or 'quantity' not in attrs):
            raise serializers.ValidationError(
...
class ReceiptDataSerializer(serializers.Serializer):
    ...
    merchant_name = serializers.CharField(required=False, allow_blank=True, max_length=255)
    transaction_time = serializers.DateTimeField(required=False)
    merchant_address = serializers.CharField(required=False, allow_blank=True, max_length=500)
    ...
    # Cost items
    cost_items = serializers.ListField(
