...
class ReceiptParseView(APIView):
    ...
    def post(self, request):
        ...
        if (
            ...
            or (isinstance(serializer.validated_data, empty))
        ):
...
class ConfirmJobView(APIView):
    ...
    def post(self, request, job_id):
        ...
        try:
            ...
            parser_service = ParserService()
            result = parser_service.confirm_job(job_id, corrections, user_token)
            ...
            total_amount = Decimal(
                str(final_data.get("total_amount", "0.00") or "0.00")
            )
            tax_amount = Decimal(str(final_data.get("tax_amount", "0.00") or "0.00"))
            subtotal_amount = Decimal(
                str(final_data.get("subtotal_amount", "0.00") or "0.00")
            )
...
class GetAllJobs(APIView):
    ...
    def get(self, request):
        ...
        try:
            parser_service = ParserService()
            result = parser_service.get_job_list(user_token)
            return Response(result)
        except Exception as e:
            raise e
