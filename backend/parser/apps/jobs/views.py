    ...
    def post(self, request):
        ...
        if "X-Receipt-Metadata" in request.headers:
            try:
                ...
                metadata["extension_filename"] = os.path.splitext(
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                if user_id:
                    logger.info(
                        f"Authenticated request with JWT for user_id: {user_id}"
                    )
        ...
        try:
            ...
            job.update_status(
                "failed", error_message=f"Failed to process job: {str(e)}"
            )
...
class ConfirmJobView(APIView):
    ...
    def post(self, request, id):
        ...
        if corrections:
            ...
            if template.success_rate == 0:
                ...
                template.success_rate = (
                    (template.success_rate / 100 * 0.7) + (0.3)
                ) * 100
            template.save()
        ...
        logger.info(
            f"Job {job.id} confirmed successfully with GridFS ID: {job.uploaded_file.grid_id}"
        )
    ...
    def process_template_improvements(
        self, job: ProcessingJob, corrections: Dict[str, Any]
    ):
            ...
            if not cache.get(cache_key):
                ...
                if job.template_used and len(corrections.keys()):
                    ...
                    if result.get("success"):
                        ...
                        logger.error(
                            f"Template update failed: {result.get('error', 'Unknown error')}"
                        )
        ...
        if auth_header and auth_header.startswith("Bearer "):
            ...
            try:
                ...
                def serialise_joblist(job: ProcessingJob):
                    ...
                    file_path: str = temp_file_path(job)
                ...
                results = list(
                    map(
                        ...
                        ProcessingJob.objects.filter(
                            user_id=user_id,
                            status__ne="confirmed"
                        )
                    )
                )
            except jwt.InvalidTokenError as e:
                logger.warning(f"Invalid JWT token for job list: {str(e)}")
            except Exception as e:
                logger.warning(f"Failed to decode JWT token: {str(e)}")
