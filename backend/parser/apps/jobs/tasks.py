...
def cleanup_temporary_files():
    ...
    all_jobs = ProcessingJob.objects(
        status__nin=["confirmed", "discarded"], uploaded_file__ne=None
    )
                    ...
                    if now > expiration_time:
                        ...
                        try:
                            ...
                            if job.status not in ["confirmed", "discarded", "failed"]:
                                ...
                                job.update_status(
                                    "abandoned", error_message="Temporary file expired"
                                )
    ...
    try:
        ...
        if mod_time < cutoff_date:
            ...
            if job.status not in ["confirmed", "discarded", "failed"]:
                ...
                job.update_status(
                    "abandoned", error_message="Temporary file expired (7-day fallback)"
                )
...
def cleanup_old_jobs():
    ...
    # Log completion
    logger.info(
        f"Job record cleanup completed in {duration.total_seconds():.2f} seconds"
    )
