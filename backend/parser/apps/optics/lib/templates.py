...
class OCRTemplate:
    ...
    def __init__(
        self, ocr_text: str, corrected_values: Optional[OCRTemplateCorrection] = None
    ):
        """
    ...
    def _find_line_items(self) -> Tuple[int, int, List[Dict[str, Any]]]:
        ...
        if earliest_start is not None:
            ...
            logger.info(
                f"Found earliest item '{earliest_item['item_name']}' at line {start_line}"
            )
        else:
        ...
        for item in cost_items:
            ...
            for i in range(start_line, len(self.lines)):
                ...
                if item["item_name"] in self.lines[i]:
                    items_found.append(
                        {"line": i, "content": self.lines[i], "item": item}
                    )
    ...
    def _create_line_item_regex(
        self, item_lines: List[Dict[str, Any]]
    ) -> Optional[LineItemMatcher]:
        """
            ...
            for item in item_lines:
                ...
                if match:
                    ...
                    if "item_name" in expected_values:
                        ...
                        if (
                            item["item"]["item_name"]
                            not in expected_values["item_name"]
                            and expected_values["item_name"]
                            not in item["item"]["item_name"]
                        ):
                            correct_match = False

                    if "total_price" in expected_values:
                        if (
                            expected_values["total_price"]
        ...
        if (
            pattern_scores and pattern_scores[0]["coverage"] >= 0.5
        ):  # At least 50% match
            best_pattern = pattern_scores[0]
            logger.info(
                f"Selected line item pattern with {best_pattern['score']}/{len(item_lines)} matches"
            )
        ...
        if len(numbers) >= 2 and text_blocks:
            ...
            custom_pattern = (
                f".*({quantity_escaped}).*({item_escaped}).*({price_escaped}).*"
            )
            custom_groups = {"quantity": 1, "item_name": 2, "total_price": 3}
        ...
        for user_field, system_field in field_mapping.items():
            ...
            for i in search_range:
                ...
                partial_score = fuzz.partial_ratio(
                    corrected_value.lower(), line.lower()
                )
                ...
                if (
                    system_field == "merchant_address"
                    and reverse_score > 80
                    and reverse_score > best_score
                ):
            ...
            if best_line >= 0 or best_offset is not None:
                ...
                if not best_pattern:
                    if (
                        system_field
                        in ["total_amount", "tax_amount", "subtotal_amount"]
                        and self.currency_symbol
                    ):
                ...
                field_extractors[system_field] = {
                    ...
                    "is_fuzzy_haystack": is_fuzzy_haystack
                    if is_fuzzy_haystack
                    else False,
                }

                logger.info(
                    f"Found {system_field} at {'line ' + str(best_line) if best_line >= 0 else 'offset ' + str(best_offset)} with score {best_score}"
                )
    ...
    def to_model_data(self) -> Dict[str, Any]:
        ...
        for field, extractor in self.template_data["field_extractors"].items():
            ...
            if "offset_from_last_item" in extractor:
                ...
                field_extractors[field]["offset_from_last_item"] = int(
                    extractor["offset_from_last_item"]
                )
    ...
    def extract_fields(self, template_model_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field, extractor in field_extractors.items():
            ...
            if line is not None and 0 <= line < len(self.lines):
                ...
                if (
                    field in ["total_amount", "tax_amount", "subtotal_amount"]
                    and self.currency_symbol
                ):
                    ...
                    for pattern in patterns:
                        if "\\d+\\.\\d{2}" in pattern:
                            symbol_pattern = pattern.replace(
                                "(\\d+\\.\\d{2})",
                    ...
                    if is_fuzzy_haystack:
                        ...
                        for i in range(line, min(line + 5, len(self.lines))):
                            score = fuzz.partial_ratio(
                                self.lines[i].lower(), haystack.lower()
                            )
        ...
        if has_line_items and item_patterns:
            ...
            if start_line is None:
                for field, extractor in field_extractors.items():
                    if (
                        field == "line_items_start"
                        and extractor.get("line") is not None
                    ):
            ...
            if start_line is None and item_patterns:
                ...
                if self.currency_symbol and "\\d+\\.\\d{2}" in patterns[0]:
                    symbol_pattern = patterns[0].replace(
                        "(\\d+\\.\\d{2})",
                        f"(?:{re.escape(self.currency_symbol)})?\\s*(\\d+\\.\\d{2})",
                    )
                            ...
                            if match:
                                ...
                                for field, group_idx in pattern_def["groups"].items():
                                    if (
                                        ...
                                        and len(match.groups()) >= group_idx
                                    ):
        ...
        if "_last_item_line" in extracted_data:
            ...
            for field, extractor in field_extractors.items():
                if field in extracted_data or not extractor.get(
                    "expected_present", True
                ):
                    continue
                    ...
                    if 0 <= target_line < len(self.lines):
                        ...
                        for pattern in patterns:
                            ...
                            if is_fuzzy_haystack:
                                ...
                                for i in range(
                                    target_line, min(target_line + 5, len(self.lines))
                                ):
                                    score = fuzz.partial_ratio(
                                        self.lines[i].lower(), haystack.lower()
                                    )
