...
class OCRProcessor:
    ...
    def _process_image(
        self, image_path: Optional[str] = None, image_pil: Optional[Image.Image] = None
    ):
        """
        ...
        try:
            ...
            if "error" in result:
                raise OCRProcessorError(
                    f"Failed to parse receipt: {result.get('error', 'Unknown error')}"
                )

            # Get data from response
            extracted_data = result.get("extracted_data", {})
            template_correspondence = (
                result.get("correspondence", 0) / 100
            ...
            if self.job:
                ...
                if "metadata" in dir(self.job) and hasattr(self.job, "metadata"):
                    ...
                    self.job.metadata["ocr_text_preprocessed"] = (
                        TemplateSuite.preprocess_ocr_text(ocr_text)
                    )
